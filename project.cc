...
int main(int argc, char *argv[]) {
    ...
    RF_AP_node.Create(RF_AP);
    initRF_AP(RF_AP_node,AP_node_list);
    ...
    Simulator::Schedule(Seconds(0.0),&printUE,UE_node_list);
    Simulator::Schedule(Seconds(0.0),&calculate_Channel_Gain_Matrix);
    Simulator::Schedule(Seconds(0.0),&DS_algo);
    Simulator::Schedule(Seconds(0.0),&calculate_SSS_throughput);
    Simulator::Schedule(Seconds(0.0),&ILB_method);
    Simulator::Schedule(Seconds(0.0),&calculate_ILB_throughput);
    //Simulator::Schedule(Seconds(5.0),&printAP_UE_Channel_gain_matrix);
    Simulator::Stop(Seconds(simulation_time));//??
    Simulator::Run();

    //std::cout<<calculate_SSS_jains_fairness()<<std::endl;
    ...
    {
        //outFile<<std::endl;
        outFile<<throughput<<','<<ILB_throughput<<',';//throughput
        outFile<<calculate_SSS_jains_fairness()<<','<<calculate_ILB_jains_fairness()<<',';
        outFile<<countDSHH()<<','<<countDSVH()<<','<<countILBHH()<<','<<countILBVH()<<std::endl;
        //outFile<<std::endl;
    }
    outFile.close();


    //std::cout<<Estimate_RF_Channel_Gain(RF_AP_node.Get(0),UE_nodes.Get(0))<<std::endl;
